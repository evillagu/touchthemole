...
describe('MoleButtonComponent', () => {
  ...
  describe('onHit', () => {
    const testCases = [
      { isActive: true, shouldEmit: true, description: 'when button is active' },
      { isActive: false, shouldEmit: false, description: 'when button is not active' }
    ];
  ...
  describe('template rendering', () => {
    const imageTestCases = [
      { isActive: true, shouldShow: true, description: 'when isActive is true' },
      { isActive: false, shouldShow: false, description: 'when isActive is false' }
    ];
    ...
    classTestCases.forEach(({ isActive, isHit, expectedClasses, description }) => {
      it(`should apply correct classes ${description}`, () => {
        ...
        expect(button.classList.contains('mole-button--active')).toBe(expectedClasses.active);
        expect(button.classList.contains('mole-button--hit')).toBe(expectedClasses.hit);

        if (isActive && isHit) {
          const image = fixture.nativeElement.querySelector('img');
          expect(image.classList.contains('mole-button__image--hit')).toBe(true);
        }
      });
    });
  });
