...
describe('ScoreBoardComponent', () => {
  ...
  describe('playerName input', () => {
    const playerNameTestCases = [
      { playerName: 'TestPlayer', expected: 'TestPlayer', description: 'valid player name' },
      { playerName: '', expected: '', description: 'empty string' },
      { playerName: 'Player123', expected: 'Player123', description: 'player name with numbers' }
    ];
    ...
    playerNameTestCases.forEach(({ playerName, expected, description }) => {
      it(`should display ${description}`, () => {
        ...
        const playerStats = fixture.nativeElement.querySelectorAll('.score-board__stat');
        const playerValue = playerStats[0].querySelector('.score-board__value');
    ...
    it('should update player name when input changes', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        ...
        const playerValue = fixture.nativeElement.querySelectorAll('.score-board__stat')[0]
  ...
  describe('points input', () => {
    ...
    pointsTestCases.forEach(({ points, expected, description }) => {
      it(`should display ${description}`, () => {
        ...
        const stats = fixture.nativeElement.querySelectorAll('.score-board__stat');
    ...
    it('should update points when input changes', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        ...
        const pointsValue = fixture.nativeElement.querySelectorAll('.score-board__stat')[1]
          .querySelector('.score-board__value');
        expect(pointsValue.textContent.trim()).toBe(expected);
      });
    });
  ...
  describe('template structure', () => {
    ...
    it('should have correct labels with i18n attributes', () => {
      ...
      expectedStructure.labels.forEach((expectedLabel, index) => {
        expect(labels[index].textContent.trim()).toBe(expectedLabel.text);
        expect(labels[index].getAttribute('i18n')).toBe(expectedLabel.i18n);
      });
    });
    ...
    it('should have correct value elements', () => {
      ...
      const values = fixture.nativeElement.querySelectorAll('.score-board__value');
