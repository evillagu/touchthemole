import { Component, ViewEncapsulation, inject, signal, WritableSignal, effect, OnDestroy } from '@angular/core';
...
import { listDifficulties, resolveDifficulty, GAME_CONFIG } from '../../../application/use-cases/difficulty.use-case';
...
export class GamePageComponent implements OnDestroy {
  ...
  readonly holes = Array.from({ length: GAME_CONFIG.totalHoles }, (_, index) => index);
  readonly gameState: WritableSignal<GameState>;
  readonly handleHit: () => void;
  readonly isGameStarted = signal<boolean>(false);
  readonly activeMoleIndex = signal<number | null>(null);
  ...
  constructor() {
    ...
    this.gameState = signal<GameState>(
      loadedState ?? startGame(GAME_CONFIG.defaultPlayerName, this.difficulties[0])
    );
  ...
  private moveMole(): void {
    ...
    this.activeMoleIndex.set(randomIndex);
    ...
    this.moleTimeout = setTimeout(() => {
      if (this.activeMoleIndex() === randomIndex) {
        this.activeMoleIndex.set(null);
      }
    }, visibilityTime);
  }
